...
return;var msg='[jquery.form] '+Array.prototype.join.call(arguments,'');if(window.console&&window.console.log){window.console.log(msg)}else if(window.opera&&window.opera.postError){window.opera.postError(msg)}}})(jQuery);(function(factory,jQuery,Zepto){if(typeof define==='function'&&define.amd){define(['jquery'],factory)}else if(typeof exports==='object'){module.exports=factory(require('jquery'))}else{factory(jQuery||Zepto)}}(function($){'use strict';var Mask=function(el,mask,options){var p={invalid:[],getCaret:function(){try{var sel,pos=0,ctrl=el.get(0),dSel=document.selection,cSelStart=ctrl.selectionStart;if(dSel&&navigator.appVersion.indexOf('MSIE 10')===-1){sel=dSel.createRange();sel.moveStart('character',-p.val().length);pos=sel.text.length}else if(cSelStart||cSelStart==='0'){pos=cSelStart}
...
setInterval(function(){if($.jMaskGlobals.watchDataMask){$.applyDataMask()}},globals.watchInterval)},window.jQuery,window.Zepto));window.efectTime=400;window.modalBaseTime=3000;window.url=$("link[rel='canonical']").attr("href");window.effectTime=500;window.timeOut=3000;$(function(){function url(uri=null){return window.url+(uri?uri:'')}
var fileTarget;var player=$("#jquery_jplayer_1");player.jPlayer({ended:function(event){$('[data-play-note]').removeAttr("disabled")},timeupdate:function(event){if(event.jPlayer.status.currentTime>=2.5){$(this).jPlayer("stop")
$('[data-play-note]').removeAttr("disabled")}},swfPath:"./jPlayer-master/dist/jplayer/jquery.jplayer.swf",supplied:"mp3",volume:1},'jquery_jplayer_1');reLoadQuestion()
$('body').on("click",'[data-play-note]',function(){player.jPlayer("play")
$(this).attr("disabled",!0)})
$(document).keyup(function(e){if(e.which==32){e.preventDefault()
e.stopPropagation()
player.jPlayer("play")
$(this).attr("disabled",!0)
return!1}})
function putOptions(options){$.each(options.responseOptions,function(key,element){var target=$(".j_opt_target:eq("+key+")");target.text(element)
target.parents("label").parent().find("input[type='radio']").val(element)
$("input[name='soundName']").val(fileTarget)})}
$("form[name='j_response']").on("change","input[name='response']",function(e){$(this).parents("form").submit()})
$('body').on("submit","form[name='j_response']",function(e){e.preventDefault();var form=$(this)
$.ajax({url:form.attr("action"),method:'post',data:form.serialize(),dataType:"json",async:!1,beforeSend:function(e){$('.ui.basic.modal').modal('show')},success:function(response){showResponseModal(response.responseStatus)
reLoadQuestion(form)}})})
function reLoadQuestion(form=null){$.ajax({url:url("/questoes"),method:'post',data:{action:'selectQuestion'},dataType:"json",async:!1,success:function(response){fileTarget=response.midiaFile
putOptions(response)
$('.ui.basic.modal').modal('hide')}});var audioFolder=url("/uploads/"+encodeURIComponent(fileTarget))
player.jPlayer("setMedia",{title:"Ouça, Memorize, Busque Referencias e Responsa",mp3:audioFolder});$('[data-play-note]').removeAttr("disabled")
if(form!=null){form[0].reset()}
setTimeout(()=>{hiddenResponseModal()},window.timeOut)}
function showResponseModal(status){var icons=(status?"fa-grin-stars":"fa-dizzy")
...
function hiddenResponseModal(){var modalBox=$('.my_alert')
var modalboxBg=$('.my_alert_mensage_box')
modalBox.fadeOut(window.efectTime,function(){modalboxBg.removeClass("sad").removeClass("happy")
modalboxBg.html('')})}})